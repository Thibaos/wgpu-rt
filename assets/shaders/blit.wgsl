// Blit shader: copies the ray tracing output texture to the screen.
// Draws a full-screen triangle (3 vertices, no vertex buffer).

struct VertexOutput {
    @builtin(position) position: vec4<f32>,
    @location(0) tex_coords: vec2<f32>,
}

@vertex
fn vs_main(@builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    var result: VertexOutput;
    let x = i32(vertex_index) / 2;
    let y = i32(vertex_index) & 1;
    let tc = vec2<f32>(f32(x) * 2.0, f32(y) * 2.0);
    result.position = vec4<f32>(tc.x * 2.0 - 1.0, 1.0 - tc.y * 2.0, 0.0, 1.0);
    result.tex_coords = tc;
    return result;
}

@group(0) @binding(0) var r_color: texture_2d<f32>;

@fragment
fn fs_main(vertex: VertexOutput) -> @location(0) vec4<f32> {
    let uv = vertex.tex_coords;
    let dims = vec2<f32>(textureDimensions(r_color));
    let px = vec2<u32>(uv * dims);
    return textureLoad(r_color, px, 0);
}
